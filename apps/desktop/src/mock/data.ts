...
export interface ModeRow {
  ...
  provider: ProviderId
  /**
   * Saniye cinsinden gecikme, ya da akış modunda sabit süre olmadığı için
   * `'stream'`. Birim metni dile göre arayüzde eklenir — sayıyı burada
   * biçimlendirmek İngilizce arayüzde Türkçe "sn" yazılmasına yol açıyordu.
   */
  latency: number | 'stream'
  /** Gecikme iyi mi (yeşil) yoksa yavaş mı (turuncu)? Akışta ölçüt yok. */
...
export const MODE_ROWS: readonly ModeRow[] = [
  ...
  {
    id: 'meeting',
    mode: 'mode.meetingSummary',
    model: 'whisper-large-v3 + haiku',
    provider: 'hybrid',
    latency: 'stream',
    latencyOk: null,
    shortcut: 'Ctrl+Alt+M',
